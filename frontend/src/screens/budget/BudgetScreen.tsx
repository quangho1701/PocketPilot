...
import type { Budget, BudgetAllocation, BudgetCategory, BudgetProgress, BudgetingMode, PlanGoal } from '@/types/budget';
import type { FinancialSetupPayload } from '@/types/setup';
import { getPrimaryGoal } from '@/types/setup';
import { approveBudgetProposal, createPlanGoal, generateBudgetProposal, getBudgetMonth, getBudgetProgress, getDraftGoals, listBudgetCategories, listPlanGoals, updatePlanGoal } from '@/services/budget';
...
export default function BudgetScreen({ navigation }: Props) {
  ...
  if (!budget) {
    ...
    if (selectedMonth.month !== now.month || selectedMonth.year !== now.year) return <StateView icon="calendar-outline" title="Chưa có kế hoạch cho tháng này" description="Không tìm thấy kế hoạch đã lưu cho tháng bạn chọn." action="Trở về tháng hiện tại" onAction={() => setSelectedMonth(now)} />;
  ...
  const primaryGoal = getPrimaryGoal(setup);
  const goalPercent = primaryGoal && primaryGoal.target_amount > 0 ? clamp((primaryGoal.current_amount / primaryGoal.target_amount) * 100) : 0;
  ...
  return <SafeAreaView style={styles.safeArea} edges={['top']}><ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
    <View style={styles.monthNav}><Pressable onPress={() => shiftMonth(-1)} accessibilityLabel="Tháng trước"><Ionicons name="chevron-back" size={20} color={COLORS.inkSoft} /></Pressable><View><Text style={[styles.eyebrow, { textAlign: 'center' }]}>SỔ KẾ HOẠCH</Text><Text style={styles.title}>{monthLabel}</Text></View><Pressable onPress={() => shiftMonth(1)} accessibilityLabel="Tháng sau"><Ionicons name="chevron-forward" size={20} color={COLORS.inkSoft} /></Pressable></View>
...
function GoalSection({ fallbackPercent }: { fallbackPercent: number }) {
  const [goals, setGoals] = useState<PlanGoal[]>([]); const [editing, setEditing] = useState<PlanGoal | 'new' | null>(null); const [name, setName] = useState(''); const [target, setTarget] = useState(''); const [deadline, setDeadline] = useState(''); const [saving, setSaving] = useState(false);
  const load = useCallback(() => { void listPlanGoals().then(setGoals); }, []); useEffect(load, [load]);
  const open = (goal: PlanGoal | 'new') => { setEditing(goal); setName(goal === 'new' ? '' : goal.name); setTarget(goal === 'new' ? '' : String(Math.round(goal.target_amount))); setDeadline(goal === 'new' ? '' : goal.target_date?.slice(0, 7) ?? ''); };
  const save = async () => { const amount = Number(target.replace(/[^0-9]/g, '')); if (!name.trim() || !amount) return; setSaving(true); try { const payload = { name: name.trim(), target_amount: amount, target_date: deadline ? `${deadline}-01` : null }; if (editing === 'new') await createPlanGoal(payload); else if (editing) await updatePlanGoal(editing.id, payload); setEditing(null); load(); } finally { setSaving(false); } };
  return <View style={{marginTop:24}}><View style={[styles.sectionHeader,{marginTop:0}]}><Text style={styles.sectionTitle}>Goals</Text><Pressable style={{flexDirection:'row',alignItems:'center'}} onPress={() => open('new')}><Ionicons name="add" size={15} color={COLORS.ink}/><Text style={styles.sectionMeta}> Add Goal</Text></Pressable></View>{goals.map((goal) => { const percent = goal.target_amount ? clamp(goal.current_amount / goal.target_amount * 100) : fallbackPercent; return <Pressable key={goal.id} style={styles.goal} onPress={() => open(goal)}><View style={styles.goalHead}><Ionicons name="flag-outline" size={20} color={COLORS.greenDeep}/><View style={styles.goalCopy}><Text style={styles.eyebrow}>{goal.is_primary?'PRIMARY GOAL':'SAVINGS GOAL'}</Text><Text style={styles.goalName}>{goal.name}</Text></View><Text style={styles.goalPercent}>{Math.round(percent)}%</Text><Ionicons name="create-outline" size={14} color={COLORS.inkSoft}/></View><View style={styles.goalTrack}><View style={[styles.goalFill,{width:`${percent}%`}]}/></View><View style={styles.goalAmounts}><Text style={styles.goalSaved}>Saved {formatCompactVnd(goal.current_amount)}</Text><Text style={styles.goalTarget}>Target {formatCompactVnd(goal.target_amount)}</Text></View></Pressable>})}<Modal visible={editing !== null} transparent animationType="fade"><View style={styles.modalBackdrop}><View style={styles.modal}><View style={styles.modalHeader}><Text style={styles.modalName}>{editing==='new'?'Add Goal':'Edit Goal'}</Text><Pressable onPress={()=>setEditing(null)}><Ionicons name="close" size={22} color={COLORS.inkSoft}/></Pressable></View><Text style={styles.inputLabel}>GOAL NAME</Text><TextInput value={name} onChangeText={setName} placeholder="e.g. New Laptop" style={styles.goalEditorInput}/><Text style={styles.inputLabel}>TARGET AMOUNT</Text><View style={styles.amountInputRow}><Text style={styles.currency}>₫</Text><TextInput value={target} onChangeText={v=>setTarget(v.replace(/[^0-9]/g,''))} keyboardType="number-pad" style={styles.amountInput}/></View><Text style={styles.inputLabel}>DEADLINE (YYYY-MM)</Text><TextInput value={deadline} onChangeText={setDeadline} placeholder="2027-03" style={styles.goalEditorInput}/>{editing!=='new'&&editing?<LedgerRow label="Already saved" value={formatVnd(editing.current_amount)} last/>:null}<Pressable onPress={()=>void save()} disabled={saving} style={[styles.primaryButton,styles.saveButton]}>{saving?<ActivityIndicator color={COLORS.parchment}/>:<Text style={styles.primaryText}>Save Goal</Text>}</Pressable></View></View></Modal></View>;
}
